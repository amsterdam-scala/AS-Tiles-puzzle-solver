    ...
    new GridPanel(Directions.values.size, Directions.values.size) {
      ...
      def buttonsSeq = for {
        ...
        def butFingerprint(x: Directi, y: Directi) = {
          val ret = if (x == y) (None, blancImg) else (Option(Tile(x, y)), getImage(Tile(x, y)))
...
object TilesSolverW extends ViewTilesSolver {
  ...
  def ui(toolbar: Option[Component] = None) = new BorderPanel() with ModelTilesSolver {
    ...
    layout(ViewTilesSolver.mainPanel) = BorderPanel.Position.Center
